...
int main()
{
    ...
    for (int i = pointX; i < img_h; i++)
    {
        for (int j = pointY; j < img_w; j++)
        {
            if (mrkMapArr[j][i]>250)
                mrkMapArr[j][i] = imgArr[j][i];
            else
                mrkMapArr[j][i] = (float(imgArr[j][i]) * 0.5 + float(mrkMapArr[j][i])*0.5);
        }
    }

    for (int i = 0; i < img_h - pointX; i++)
    {
        for (int j = 0; j < img_w - pointY; j++)
        {
            newImgArr[j][i] = mrkMapArr[j+pointX][i+pointY];
        }
    }  

    FILE *fp_w = fopen("res\\cover.bmp", "wb");
