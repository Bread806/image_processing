...
int main()
{
    ...
    for (int i = 0; i < img_h; i++)
    {
        ...
        {
            if      (imgArr[j][i] < 100)imgArr[j][i]=First(imgArr[j][i]);
            else if (imgArr[j][i] >= 100 && imgArr[j][i] < 150)imgArr[j][i]=Second(imgArr[j][i]);
            else    imgArr[j][i]=Third(imgArr[j][i]);
        }
    }

    FILE *fp_w = fopen("res\\enhencement_img.bmp", "wb");
